...
if not status is-interactive; exit;
end 
...
alias psmem='ps auxf |
	sort -nr -k 4'; alias pscpu='ps auxf | sort -nr -k 3' 

# --- Git Aliases ---
alias addup='git add -u';
alias addall='git add .'; alias branch='git branch' 
alias checkout='git checkout'; alias clone='git clone'; alias commit='git commit -m'
alias fetch='git fetch';
alias pull='git pull origin'; alias push='git push origin' 
alias stat='git status'; alias tag='git tag';
...
if command -v fd > /dev/null; alias find='fd';
end 
...
function refresh
	set -l C_PATH ~/.config/fish/config.fish;
	set -l D_DIR (dirname (readlink -f $C_PATH)) 
