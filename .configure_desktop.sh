...
# 1. Safety Check for Kitty
if ! command -v kitty >/dev/null 2>&1; then
	print_error "Kitty is not installed or not in PATH. Cannot set as default terminal."
...
print_info "Detected Desktop Environment: $CURRENT_DE"
...
case "$CURRENT_DE" in
	*gnome*|*unity*)
		...
		if command -v gsettings >/dev/null 2>&1; then
			...
			# 2. Font Defaults (Candara for UI, Inconsolata for Mono)
			...
			print_info "GNOME terminal and font settings updated."
		else
			print_warning "gsettings not found."
		fi
		;;
		...
		if command -v kwriteconfig5 >/dev/null 2>&1; then
            # 1. Terminal Defaults
             if [ "$KITTY_AVAILABLE" = true ]; then
			    ...
			    kwriteconfig5 --file ~/.config/kdedefaults/kdeglobals --group General --key TerminalApplication kitty
             fi
			print_info "KDE terminal settings updated. (Fonts rely on system-wide .fonts.conf)"
		else
			print_warning "kwriteconfig5 not found."
		fi
		;;
	...
	*xfce*)
		...
		if command -v xfconf-query >/dev/null 2>&1; then
			...
			print_info "XFCE terminal and font settings updated."
		else
			print_warning "xfconf-query not found."
		fi
		;;

	*)
		print_warning "Unknown DE: $CURRENT_DE. Skipping DE-specific settings."
		;;
esac

# --- Final Fallback: Debian Alternatives ---
if [ -f /etc/debian_version ] && command -v update-alternatives >/dev/null 2>&1; then
	if [ "$EUID" -ne 0 ] && ! sudo -n true >/dev/null 2>&1; then
		print_warning "Not root. Skipping update-alternatives."
	elif [ "$KITTY_AVAILABLE" = true ]; then
		print_info "Setting Debian alternatives priority..."
		sudo update-alternatives --install /usr/bin/x-terminal-emulator x-terminal-emulator "$(which kitty)" 50 2>/dev/null
	fi
fi
