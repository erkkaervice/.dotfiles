...
### Core Terminal & Desktop Experience
...
- **Desktop Integration**: A specialized script (`.configure_desktop.sh`) automatically sets Kitty as the default terminal, applies font settings, and **automatically applies your custom wallpaper** on GNOME, KDE Plasma, XFCE, and standalone Window Managers (via `feh`).
- **NOTE:** This setup _requires you to provide your own font and wallpaper files_ (see Installation).
...
- **Git-Aware**: Displays branch and status indicators (`U` for untracked/unstaged, `+` for staged) only when inside a repository.
...
- **`startfresh`**: A "factory reset" that wipes all symlinked configs and local apps, resetting to a clean system shell. **(Safely preserves your `~/.config/shell_secrets` file).**
...
- **`ipinfo`**: Quick public IP and domain lookup via HTTPS.
...
- **Robust Installation**: `setup.sh` detects the OS, handles `sudo`/non-`sudo` scenarios, and installs packages via resilient loops.
...
- **SSH Persistence**: Includes `.ssh_agent_init` to keep `ssh-agent` running across sessions securely.
...
- **Core Tools Installed**: The setup script attempts to install `nmap`, `gnupg`, `lynis`, `tcpdump`, `trivy`, and `gitleaks`.
  - _(Note: `trivy` and `gitleaks` are omitted on Debian/Ubuntu by default as they require third-party repositories)._
  - On **Termux**, `nmap` and `gnupg` are installed successfully.
  - On **sudo-less** machines, system security tools are **not** installed (see Troubleshooting).
...
- **Debian Family:** Debian, Ubuntu, Kali Linux, Tails (Mint, Pop!\_OS)
- **Arch Family:** Arch Linux, SystemRescue, SteamOS (Manjaro)
- **Fedora Family:** Fedora, RHEL, CentOS
- **NixOS** (Installs tools into user environment via `nix-env`)
- **openSUSE**
- **Alpine Linux**
- **macOS** (via Homebrew)
- **Termux** (Android)
...
3.  **Add Your Fonts & Wallpaper:**
    This setup **requires** you to provide your own fonts and wallpaper. The script will _not_ download them.
    - Place your `.ttf` files inside the `.fonts/` directory:
    ...
    - Place your background image inside the `.wallpaper/` directory and name it **`wallpaper.jpg`**.
...
4.  **Run the Setup Script:**
    ...
    - **Safely configures Git using `include.path` (allowing seamless Dev Container credential passthrough) and links `.gitignore` as `~/.gitignore_global`.**
    - Triggers desktop integration to set Kitty, apply standard fonts, and set the system wallpaper.
...
`sh
    ...
    `
Copy the _entire block_ (from `-----BEGIN...` to `-----END...`) and paste it into GitHub under **Settings > SSH and GPG keys > New GPG key**.
...
- **`.configure_desktop.sh`**: Called by `setup.sh` to apply DE-specific settings (default terminal, fonts, wallpaper).
...
- **`.fonts/`**: **(User-Provided)** Directory where you must place your `.ttf` files.
- **`.wallpaper/`**: **(User-Provided)** Directory where you must place your `wallpaper.jpg` file.
- **`.gitignore`**: Template for `~/.gitignore_global`. Prevents secrets, cache files, and OS junk from being committed _in any repo on your system_.
...
- **Wallpaper**: Replace `~/.dotfiles/.wallpaper/wallpaper.jpg` and run `refresh`.
...
- **Fonts are broken / Kitty won't start:** This almost always means you did not place the required `.ttf` files (especially `InconsolataNerdFont-Regular.ttf`) into the `.fonts/` directory _before_ running `.setup.sh`.
